<template>
  <div>
    <div class="registered">
      ...
      <div class="bored_bottom">
        <Form ref="user" :model="user" :rules="ruleInline">
          ...
          <FormItem class="m-t-25">
            <Button
              v-if="user.username===''||user.password===''||user.password1===''||flag===true"
              type="primary"
              disabled
              long
            ...
            <Button
              v-if="user.username!==''&&user.password!==''&&user.password1!==''&&flag===false"
              type="primary"
              long
      ...
      <div class="f12 f-j ps_bottom">
        <span>如果您已拥有 XMall 账号，则可在此</span>
        <span class="m-l-1 ps" @click="login">登陆</span>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    handleSubmit(name) {
      this.$refs[name].validate(valid => {
        if (valid && this.single === false) {
          this.$Message.error("请同意 XMall 商城服务使用协议");
        } else if (valid && this.single === true) {
          this.text = "注册中...";
          this.flag = true;
          this.$api
            .getRegister({username:this.user.username, password:this.user.password})
            .then(res => {
              console.log(res);
            })
            .catch(err => {
