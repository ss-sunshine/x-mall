    ...
    getSort() {
      ...
      if (this.priceGt === "" || this.priceLte === "") {
        this.$api
          ...
          .then(res => {
            this.arr = res.data;
            localStorage.setItem('search1',JSON.stringify(this.arr))
            if (this.$route.query.value) {
              let arr = JSON.parse(localStorage.getItem("search1")); 
              let arr1 = JSON.parse(localStorage.getItem("search"));
              this.arr = intersection(arr, arr1);
              console.log(this.arr);
              let a = [2, 1, 3, 4];
              let a1 = [4, 2, 5];
              let a2 = intersection(a, a1);
              console.log(a2);
            }
          })
          .catch(err => {
